...
export default function ProfilePage() {
  ...
  const { data: evaluations, isLoading: evalsLoading } = useGetEvaluations(targetUserId);
  const { data: tempStats, isLoading: tempLoading } = useUserTemperature(targetUserId);
        ...
        <div className="bg-[#18181b] border-2 border-gray-700 rounded-lg p-8 mb-6">
          <div className="flex items-start gap-6">
            ...
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-4xl font-bold flex-shrink-0">
              {(isOwnProfile ? user?.nickname : `U`)?.[0] || '?'}
            </div>

            {/* Info */}
            <div className="flex-1">
              <h1 className="text-3xl font-bold mb-2">
                {isOwnProfile ? user?.nickname : `사용자 ${userId}`}
              </h1>
