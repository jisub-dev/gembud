...
| `feat/t1-notification-live` | 🟢 병합 완료 | Terminal 1 | squash merged | 2026-03-11 14:09 |
| `feat/t2-pwa-offline` | 🟢 병합 완료 | Terminal 2 | squash merged | 2026-03-11 14:09 |
| `feat/t3-report-admin-flow` | 🟢 병합 완료 | Terminal 3 | squash merged | 2026-03-11 14:09 |
...
### 완료 보고 양식 (Claude에게 전달) ⭐ 반드시 이 형식으로
...
• 핵심 변경
  - 무엇을 왜 변경했는지 bullet로 설명

• vitest 결과 (프론트 작업 시)
  - 실행: npx vitest run --reporter=verbose 2>&1 | tail -20
  - 결과: Test Files N passed (N), Tests N passed (N)

• 백엔드 테스트 결과 (백엔드 작업 시)
  - 실행: ./gradlew test ... 2>&1 | tail -20
  - 결과: N tests passed / N failed

• build 결과 (프론트 작업 시)
  - 실행: npm run build 2>&1 | tail -10
  - 결과: ✓ built in Xs (성공) or 에러 내용

• 남은 리스크
  - 미구현 항목, 알려진 경고, 추가 작업 필요 사항

• 추가
  - 커밋: (hash)
  - 푸시: origin/feat/브랜치명
  - PR 링크: https://github.com/jisub-dev/gembud/pull/new/feat/브랜치명
```
